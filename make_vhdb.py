...
def load_eboot_cache():
    path = os.path.join(REPO, EBOOT_CACHE)
    if not os.path.exists(path):
        return {}
    with open(path, "r", encoding="utf-8") as handle:
...
def build():
    ...
    for filename, platform, label in SOURCES:
        ...
        for entry in entries:
            ...
            known = eboots.get(str(get("id") or ""), {})
            eboot, valid_eboot = pack_md5(known.get("eboot"))
            aux, valid_aux = pack_md5(known.get("aux"))
            aux_kind = to_int(known.get("aux_kind")) & 0xFF
            if valid_eboot:
                flags |= FLAG_HAS_EBOOT
            if valid_aux:
                flags |= FLAG_HAS_AUX
            else:
                aux_kind = 0
